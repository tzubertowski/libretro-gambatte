...
#ifdef SF2000
static unsigned sf2000_fastforward_state        = 0;  /* 0=1x, 1=2x, 2=3x */
...
static const unsigned char splash_font_8x8[][8] = {
   ...
   {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}, // / (47) - unused
...
static void update_input_state(void)
{
   ...
   if (libretro_supports_bitmasks)
   {
      ...
      /* On SF2000, A button is mapped to 0x100 which is RETRO_DEVICE_ID_JOYPAD_A (button 8) */
      bool select_pressed = (ret & (1 << RETRO_DEVICE_ID_JOYPAD_SELECT));
      bool a_pressed = (ret & 0x100);  /* Direct bit check for SF2000 A button */
      bool sf2000_select_a_current = select_pressed && a_pressed;
      
      /* Always log when either button is pressed for debugging */
      if (select_pressed || a_pressed)
      {
         gambatte_log(RETRO_LOG_INFO, "SF2000 Button Debug: SELECT=%d, A(0x100)=%d, combo=%d\n", 
                      select_pressed, a_pressed, sf2000_select_a_current);
      }
      ...
      if (sf2000_select_a_current && !sf2000_select_a_prev)
      {
         ...
         gambatte_log(RETRO_LOG_ERROR, "*** SF2000 Fast Forward: Switched to speed state %u (multiplier: %ux) ***\n", 
                      sf2000_fastforward_state, sf2000_fastforward_state + 1);
      ...
      if (sf2000_select_b_current && !sf2000_select_b_prev)
      {
         ...
         const char* speed_labels[] = {"1x", "0.5x", "0.2x"};
         gambatte_log(RETRO_LOG_ERROR, "*** SF2000 Slow Motion: Switched to speed state %u (%s) ***\n", 
                      sf2000_slowmotion_state, speed_labels[sf2000_slowmotion_state]);
      }
   ...
   else
   {
...
#ifdef SF2000
      /* SF2000: Check for SELECT+A combination for multi-speed fast forward */
      /* On SF2000, A button is mapped to RETRO_DEVICE_ID_JOYPAD_A (button 8) */
      ...
      if (select_pressed || a_pressed)
      {
         gambatte_log(RETRO_LOG_INFO, "SF2000 Button Debug (non-bitmask): SELECT=%d, A=%d, combo=%d\n", 
                      select_pressed, a_pressed, sf2000_select_a_current);
      }
...
void retro_get_system_av_info(struct retro_system_av_info *info)
{
   ...
   {
      unsigned speed_multiplier = sf2000_fastforward_state + 1; /* 2 or 3 */
...
void retro_run()
{
   ...
   if (sf2000_fastforward_state > 0)
   {
      unsigned iterations = sf2000_fastforward_state + 1; /* 2 or 3 */
...
static void retro_run_internal()
{
   ...
   else
   {
      ...
      if (sf2000_fastforward_state > 0)
      {
         unsigned iterations = sf2000_fastforward_state + 1; /* 2 or 3 iterations */
         
         /* Run the emulation multiple times but only keep the final video frame */
         for (unsigned iter = 0; iter < iterations; iter++)
         {
            samples = SOUND_SAMPLES_PER_RUN;
            gambatte::video_pixel_t *frame_buf = (iter == iterations - 1) ? video_buf : video_buf; /* Always use video_buf for now */
