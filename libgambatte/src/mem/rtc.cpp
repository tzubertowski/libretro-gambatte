...
namespace gambatte
{
   ...
   {
      if (fake_rtc_is_enabled())
      {
         struct tm fake_time;
         fake_rtc_get_time(&fake_time);
         uint64_t fake_timestamp = static_cast<uint64_t>(mktime(&fake_time));
         return fake_timestamp;
      }
      else
      {
         uint64_t sys_time = std::time(0);
         return sys_time;
      }
   }
   ...
   void Rtc::doLatch()
   {
      ...
      uint64_t tmp = ((dataDh_ & 0x40) ? haltTime_ : current_time) - baseTime_;
   ...
   {
      const uint64_t unixtime     = (dataDh_ & 0x40) ? haltTime_ : get_current_time();
      ...
      {
         if (new_dh & 0x40)
            haltTime_ = get_current_time();
         else
            baseTime_ += get_current_time() - haltTime_;
      }
   }
