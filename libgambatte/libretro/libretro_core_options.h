...
struct retro_core_option_v2_definition option_defs_us[] = {
...
#if (defined(PS2) || defined(PSP) || defined(VITA) || defined(_3DS) || defined(DINGUX) || defined(__mips__))
      "cc"
#else
      "sinc"
#endif
...
#ifdef __mips__
   {
      "gambatte_mips_performance",
      "MIPS Performance Mode",
      NULL,
      "Optimizes sprite processing for better performance on MIPS soft FPU systems. 'Fast' reduces sprite processing overhead. 'Faster' disables some cycle-accurate timing for maximum performance.",
      NULL,
      NULL,
      {
         { "disabled", "Disabled (Accurate)" },
         { "fast",     "Fast" },
         { "faster",   "Faster" },
         { NULL, NULL },
      },
   ...
   {
      "gambatte_fast_forward_audio",
      "Fast Forward/Slow Motion Audio",
      NULL,
      "Controls whether audio playback continues during fast forward and slow motion modes. When disabled, audio will be muted during speed changes to improve performance.",
      NULL,
      NULL,
      {
         { "enabled",  NULL },
...
#endif
   {
      ...
      "Fake RTC",
      NULL,
      "Enable fake Real-Time Clock for devices without hardware RTC support. Uses a software-based minute counter system to track time progression. Useful for SF2000 and similar handheld devices.",
      NULL,
      NULL,
      {
         { "enabled",  NULL },
   ...
   },
   {
      ...
      NULL,
      "Applies consistent time adjustment every emulator startup. Positive values advance time, negative values rewind time. This setting persists across sessions and is useful for continuous time progression scenarios.",
      NULL,
      NULL,
      {
         { "0",     "0 (No Change)" },
   ...
   {
      "gambatte_fake_rtc_one_off_bump_minutes",
      "One-Off Time Bump (Minutes)",
      NULL,
      "Applies time adjustment once, then automatically resets to 0. Useful for one-time corrections or special events. The setting will be reset to 0 after being applied.",
      NULL,
      NULL,
      {
         { "0",     "0 (No Change)" },
   ...
   {
      "gambatte_fake_rtc_skip_time_set",
      "Skip 'Time Not Set' Message",
      NULL,
      "When enabled, makes Pokemon games think the RTC time has already been set by the user, preventing the 'TIME NOT SET' prompt on every boot. Recommended for devices without hardware RTC.",
      NULL,
      NULL,
      {
         { "disabled", "disabled (Show time set prompt)" },
         { "enabled", "enabled (Skip time set prompt)" },
         { NULL, NULL },
      },
      "enabled"
   },
